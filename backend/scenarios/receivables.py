...
def act(item: RiskItem) -> str:
    ...
    link = razorpay_client.create_payment_link(
        ...
        description=f"Payment reminder for invoice {item.item_id}",
        reference_id=item.item_id,
        customer_name=item.customer_id,
    )
    return (
        f"Sent reminder #{_reminder_counts[item.item_id]} to {item.customer_id} "
        f"for {item.item_id} (₹{item.amount:.0f}). Payment link: {link['short_url']}"
    )
...
def reset():
    """Clears in-memory reminder counters so a fresh demo run starts clean."""
    _reminder_counts.clear()
