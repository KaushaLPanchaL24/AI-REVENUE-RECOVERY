...
def act(item: RiskItem) -> str:
    ...
    if item.action == "request_alternate_method":
        return f"Sent message to {item.customer_id} asking for an alternate payment method for {item.item_id}."
    return f"Triggered retry #{_retry_counts[item.item_id]} for {item.item_id} (₹{item.amount:.0f})."
