...
def act(item: RiskItem) -> str:
    ...
    if item.action == "send_discount_nudge":
        return f"Sent 10% discount nudge to {item.customer_id} to complete cart {item.item_id}."
    if item.action == "stop":
        return f"No further action for {item.item_id} — nudge already sent."
    return f"Sent reminder nudge to {item.customer_id} to complete cart {item.item_id} (₹{item.amount:.0f})."
